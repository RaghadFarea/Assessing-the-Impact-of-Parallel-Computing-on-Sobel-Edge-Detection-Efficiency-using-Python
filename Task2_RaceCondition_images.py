...
def sobel_single_image(image, shared_image_counter, shared_pixel_counter):

    # Shared image counter
    shared_image_counter.value = 1 + shared_image_counter.value
    # shared pixle counter
...
def process_images_in_parallel(images, shared_image_counter, shared_pixel_counter):
    print("\nThe start of parallel processing")
    num_cores = os.cpu_count()
    print(f"Using {num_cores} CPU cores for parallel processing.")
    args = [(image, shared_image_counter, shared_pixel_counter) for image in images]
    start_time = time.time()

    # Create a Pool with the number of CPU cores
...
def process_images(folder_path, main_path):
    ...
    with Manager() as manager:
        ...
        sobel_images = process_images_in_parallel(images, shared_image_counter, shared_pixel_counter)
...
def main():
    main_path = os.getcwd()
    folder_path = os.path.join(main_path, "New_Dataset\\Outerloop\\Full_Run")
