...
def sobel_single_image(image, queue):
    ...
    queue.put((image_count, pixel_count))

    return result

#########################################################################################

def process_images_in_parallel(images, queue):
    print("\nThe start of parallel processing")
    num_cores = os.cpu_count()
    print(f"Using {num_cores} CPU cores for parallel processing.")
    args = [(image, queue) for image in images]
    ...
    while not queue.empty():
        image_count, pixel_count = queue.get()
...
def process_images(folder_path, main_path):
    ...
    # Use Manager to create shared values for the counters
    with Manager() as manager:
        queue = manager.Queue() 
        sobel_images = process_images_in_parallel(images, queue)
